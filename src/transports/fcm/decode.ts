 ...
 * imports, so it is unit-testable in isolation. Restored from the original FCM
 * plugin (`main`'s src/inbound-core.ts), plus the branch fields DirectPusher
 * has added since (reply-to-recipient, bot sender, media).
 *
 ...
 *       is_mention_of_recipient, is_everyone_mention, is_reply_to_recipient,
 *       sender_is_agent, has_media, key, target_event_id,
 *     },
 *   }
 *
...
export interface DecodedPush {
  ...
  /** A reply to one of the recipient's own messages. */
  isReplyToRecipient?: boolean;
  /** The sender is a bot user — another agent. */
